...
def run_pipeline_RV_no_OT(cfg: DictConfig) -> None:
    ...
    rotational = calculate_rotational_advanced(full, new_border, initial_apex[0], rotational_reference[0], center, smooth_iter=iter_smooth_rot)
    ...
    alpha_values = get_alpha_values(transmural, alpha_endo=cfg.angles.alpha_RV_endo, alpha_epi=cfg.angles.alpha_RV_epi)
    beta_values=None
    if cfg.calculate_transverse or cfg.calculate_sheet:
        logger.info("Calculating beta values")
        beta_values = get_beta_values(transmural, beta_endo=cfg.angles.beta_RV_endo, beta_epi=cfg.angles.beta_RV_epi)
